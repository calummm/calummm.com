...
let canvas = document.getElementById('jsBob2Canvas');
const viewWidth = 500;
...
const playerData = levelData.objects.find((object) => object.type === 'player');
const platformData = levelData.objects.filter((object) =>
	['ground', 'platform', 'block', 'wall'].includes(object.type)
);
const enemyData = levelData.objects.filter((object) => object.type === 'enemy');
...
const draw = () => {
	...
	if (ctx) {
		...
		platformData.forEach((platform) => {
			...
			ctx.fillRect(platform.x, platform.y, platform.w ?? 0, platform.h ?? 0);
		});
		ctx.restore();
